...
defineTypeNameAndDebug(TangDragFace, 0);
...
void TangDragFace::setForce() const
{
    // get mesh
    ...
    if (scaleDia_ > 1)
    {
        Info << "TangDragFace using scale = " << scaleDia_ << endl;
    }
    else if (particleCloud_.cg() > 1)
    {
        scaleDia_=particleCloud_.cg();
        Info << "TangDragFace using scale from liggghts cg = " << scaleDia_ << endl;
    }
...
double TangDragFace::F(double voidfraction, double Rep) const
{
