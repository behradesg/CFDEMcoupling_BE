...
defineTypeNameAndDebug(WenYuDragFace, 0);
...
void WenYuDragFace::setForce() const
{
    // get mesh
    ...
    if (scaleDia_ > 1)
    {
        Info << "WenYuDragFace using scale = " << scaleDia_ << endl;
    }
    else if (particleCloud_.cg() > 1)
    {
        scaleDia_=particleCloud_.cg();
        Info << "WenYuDragFace using scale from liggghts cg = " << scaleDia_ << endl;
    }
...
double WenYuDragFace::F(double voidfraction, double Rep) const
{
    ...
    return   F0*Foam::pow(voidfraction, -3.65);
