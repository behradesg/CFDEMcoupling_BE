...
defineTypeNameAndDebug(VanWachemDrag, 0);
...
void VanWachemDrag::setForce() const
{
    if (scaleDia_ > 1)
    {
        Info << "VanWachemDrag using scale = " << scaleDia_ << endl;
    }
    else if (particleCloud_.cg() > 1)
    {
        scaleDia_=particleCloud_.cg();
        Info << "VanWachemDrag using scale from liggghts cg = " << scaleDia_ << endl;
    }
    ...
    interpolationCellPoint2<scalar> voidfractionInterpolator_(voidfraction_);
    interpolationCellPointWallModified<vector> UInterpolator_(U_);
...
double VanWachemDrag::F(double voidfraction, double Rep) const
{
    double localPhiP
    (
        Foam::max
