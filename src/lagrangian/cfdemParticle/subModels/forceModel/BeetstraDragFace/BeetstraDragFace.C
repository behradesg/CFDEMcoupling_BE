...
defineTypeNameAndDebug(BeetstraDragFace, 0);
...
BeetstraDragFace::BeetstraDragFace
(
...
void BeetstraDragFace::setForce() const
{
    // get mesh
    ...
    scalar voidfraction(scalar(1.0));
    ...
    scalar ds(scalar(0.0));
    scalar ds_scaled(scalar(0.0));
    scalar dSauter(scalar(0.0));
    scalar scaleDia3 = scaleDia_*scaleDia_*scaleDia_;
    scalar nuf(scalar(0.0));
    scalar rho(scalar(0.0));
    scalar magUr(scalar(0.0));
    scalar Rep(scalar(0.0));
    scalar localPhiP(scalar(0.0));
    scalar GCcorr(scalar(1.0));
    scalar PCcorr(scalar(1.0));

    scalar cg = scaleDia_;
    label partType = 1;

    vector dragExplicit(vector::zero);
    scalar dragCoefficient(scalar(0.0));
    ...
    forAll(voidFractionList, i) {
        voidFractionList[i] = 0.0;
        listUf[i]           = vector::zero;
    }

    // internalField
    forAll(voidFractionFace,i)
    {
    ...
    for(int index = 0; index < particleCloud_.numberOfParticles(); ++index)
    {
            ...
            if (cellI > -1) // particle found
            {
                ...
                Rep=dSauter*voidfraction*magUr/(nuf + SMALL) + SMALL;
                ...
                localPhiP = Foam::min(1.0f-voidfraction+SMALL,0.6); // Radl & Sundaresan (2014)
                if (useGC_)
                {
                    GCcorr = 1.0-h(localPhiP)
                                /(  a(localPhiP)
                                    *Lc2_
 ...
 *********************************************************/
double BeetstraDragFace::F(double voidfraction, double Rep) const
{
 ...
 *********************************************************/
double BeetstraDragFace::a(double phiP) const
{
    double a0m = 0.;
    double a1m = 0.;
    double a2m = 0.;
...
double BeetstraDragFace::h(double phiP) const
{
  double h0m = 0.;
  double h1m = 0.;
  double h2m = 0.;
  ...
  return 
    Foam::max
    (
        h0m + h1m*phiP_minus_phiphm + h2m*phiP_minus_phiphm*phiP_minus_phiphm
...
double BeetstraDragFace::terminalVelocity(double voidfraction, double dp, double nuf, double rhof, double rhop, double g) const
{
