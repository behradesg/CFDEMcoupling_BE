    ...
    )
{
    ...
    Info << "implicit momentum exchange field calculate if voidFraction is larger than : " <<  maxAlpha_ << endl;
}
...
tmp<volScalarField> implicitCouple::CdS()
{
    const scalar tsf = particleCloud_.dataExchangeM().timeStepFraction();
    if(tsf < particleCloud_.mesh().time().deltaT().value()/particleCloud_.dataExchangeM().couplingTime() + 0.000001 )
    {
        forAll(KslNext_,cellI)
        {
            if (alpha_[cellI] < maxAlpha_) 
            {
                KslNext_[cellI] = particleCloud_.forceM(0).CdS()[cellI]
                                / particleCloud_.mesh().V()[cellI];
            }
            else 
            {
