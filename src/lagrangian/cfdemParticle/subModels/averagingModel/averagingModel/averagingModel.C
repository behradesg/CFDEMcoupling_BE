...
void averagingModel::resetScalarAverage(volScalarField& prev,volScalarField& next,bool single) const
{
    if(!single) prev.ref() = next.ref();
    next.primitiveFieldRef() = scalar(0.0);
}
...
:
    ...
    //    sm.mesh().lookupObject<volVectorField> ("Us")
        sm.mesh(),
        dimensionedVector("zero", dimensionSet(0,1,-1,0,0),vector::zero),
        sm.mesh().lookupObject<volVectorField> ("Us").boundaryField().types()
       // zeroGradientFvPatchField<vector>::typeName // apply zeroGradient for UsNext for appropriate smoothing
    )
{
