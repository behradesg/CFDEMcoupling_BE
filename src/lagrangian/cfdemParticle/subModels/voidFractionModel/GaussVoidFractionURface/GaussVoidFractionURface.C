...
#include "error.H"
#include "mathExtra.H"
#include "GaussVoidFractionURface.H"
...
defineTypeNameAndDebug(GaussVoidFractionURface, 0);

addToRunTimeSelectionTable
(
    voidFractionModel,
    GaussVoidFractionURface,
    dictionary
);
...
GaussVoidFractionURface::GaussVoidFractionURface
(
...
void GaussVoidFractionURface::setvoidFraction(double** const& mask,double**& voidfractions,double**& particleWeights,double**& particleVolumes,double**& particleV)
{
    // get mesh
    ...
    for(int index=0; index< particleCloud_.numberOfParticles(); index++)
    {
        ...
        //{
            ...
            if (particleCenterCellID >= 0)
            {
                ...
                scalar cellSize(Foam::cbrt(mesh.V()[particleCenterCellID]));
                scalar variance(stencil_*stencil_*cellSize*cellSize);
                ...
                forAll(mesh.cells()[particleCenterCellID], facei)
                {
                    ...
                    dist2 = Foam::magSqr(listCf[newFaceI] - positionCenter);
                    core = Foam::exp(-0.5*dist2/variance);
                    normalization += core;
                    hits++;
                }
